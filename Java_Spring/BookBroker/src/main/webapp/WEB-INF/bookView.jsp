						...
						<c:choose>
							<c:when test="${book.postedBy.id == userId}">
								<c:choose> 
									...
									<c:otherwise>
										<p class="text-end mb-0 p-1">Currently borrowed to <c:out value="${book.borrowedBy.firstName}"/> <c:out value="${book.borrowedBy.lastName}"/></p>
									</c:otherwise>
								</c:choose>
							</c:when>
							<c:otherwise>
								...
								<c:choose> 
									<c:when test="${book.borrowedBy == null}">
										<form action="/books/${book.id}/borrow" method="post">
											...
											<input type="submit" class="btn btn-success mt-3" value="Borrow"/>										 
										 </form>
									</c:when>
									<c:otherwise>
										<p class="text-end mb-0 p-1">Currently borrowed to <c:out value="${book.borrowedBy.firstName}"/> <c:out value="${book.borrowedBy.lastName}"/></p>
